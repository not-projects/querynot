...
QueryNot is a local-first desktop SQL client from Not Projects. Version 0.1.3 is the current published Windows 11 x86-64 release on the signed-updater channel; the source tree is prepared as 0.1.4 pending signed candidate validation and publication.
...
- `npm run release:prepare-update-publication -- --directory <candidate-dir> --output artifacts/publication --tag v0.1.4 --confirm publish-v0.1.4 --report <report>` — verify the candidate source, retained reports, hashes, manifest, and updater signature, then stage exactly the installer, signature, `latest.json`, and checksum.
- `npm run release:verify-update-publication -- --directory <download-dir> --tag v0.1.4 --report <report>` — cryptographically and byte-for-byte verify the four draft or public assets; the manual signed-release workflow runs this before publication.
