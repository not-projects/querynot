...
QueryNot `0.1.7` is the current public release for Windows 11 x86-64, Linux x86-64, macOS Intel, and macOS Apple silicon under one signed updater contract. The source tree is prepared as `0.1.8` pending signed candidate validation and publication. Security fixes are applied on a best-effort basis to the current `master` branch and supported release line.
