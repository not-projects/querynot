...
Phase 5 now also retains the passing local-automation, dependency, exact five-server adapter, Linux artifact-inspection, and Linux checksum reports for one exact candidate source commit. These files prove only their stated local gates. They do not turn template files into evidence or satisfy an operating-system package row without native install/core-journey/uninstall results.
