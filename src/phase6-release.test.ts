...
describe('Phase 6 publication boundary', () => {
  ...
  it('accepts only the exact signed cross-platform updater assets and feed keys', () => {
    const names = {
      ...
      'macos-dmg-x64': 'QueryNot_0.1.10_x64.dmg',
      'macos-dmg-aarch64': 'QueryNot_0.1.10_aarch64.dmg',
        ...
        updaterPlatformBindings.map(({ key, payloadId }) => {
          ...
          return [
            key,
            {
              ...
              url: `https://github.com/not-projects/querynot/releases/download/v0.1.10/${payload.name}`
            }
          ];
        })
      )
