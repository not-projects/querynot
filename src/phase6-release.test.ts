...
describe('Phase 6 publication boundary', () => {
  ...
  it('accepts only the exact signed cross-platform updater assets and feed keys', () => {
    const names = {
      'windows-nsis-x64': 'QueryNot_0.1.15_x64-setup.exe',
      'windows-msi-x64': 'QueryNot_0.1.15_x64_en-US.msi',
      'linux-appimage-x64': 'QueryNot_0.1.15_amd64.AppImage',
      'linux-deb-x64': 'QueryNot_0.1.15_amd64.deb',
      'linux-rpm-x64': 'QueryNot-0.1.15-1.x86_64.rpm',
      'macos-dmg-x64': 'QueryNot_0.1.15_x64.dmg',
      'macos-dmg-aarch64': 'QueryNot_0.1.15_aarch64.dmg',
        ...
        updaterPlatformBindings.map(({ key, payloadId }) => {
          ...
          return [
            key,
            {
              ...
              url: `https://github.com/not-projects/querynot/releases/download/v0.1.15/${payload.name}`
            }
          ];
        })
      )
