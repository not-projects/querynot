...
Icon-only controls use one dependency-free inline SVG component with `currentColor` styling. SVG paths remain decorative inside labelled buttons, summaries, or status elements. The schema tree uses those icons for disclosure and object kinds, and selecting a table or view makes its existing columns, types, primary keys, defaults, generated fields, indexes, and foreign keys the primary detail. The former **Data** row action becomes the explicit **Open rows** action inside that detail, keeping row browsing distinct from structural inspection.
...
- Structure-first schema detail uses the existing adapter contract and makes row browsing explicit.
...
Unit and integration tests cover literal Control routing, stable Settings-dialog scale, viewport-bounded modal CSS, shared SVG icon usage, structure-first schema detail, and existing tab/session safeguards. The populated Chromium fixture executes through `Ctrl+Enter`, inspects columns, keys, indexes, and foreign keys before opening rows, and checks the compact header, two toolbar groups, concise status help, readable connection rows, scoped tabs, result visibility, split geometry, History overlay, responsive widths, and 150% application scaling. A separate 200%-scale Settings check uses a 600px-high viewport and scrolls to both the bottom Save action and top Close control.
