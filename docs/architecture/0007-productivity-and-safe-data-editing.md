...
CodeMirror receives the adapter dialect and available schema metadata for keywords, built-ins, qualified objects, columns, and current-statement aliases. When a connection or query-tab context becomes active, the existing native metadata session preloads that database/schema's relation names and a bounded set of table/view columns without opening an execution session. Completion resolves unqualified names against the query-tab context, reuses preloaded structures, and requests missing referenced objects on demand when a catalog exceeds the preload bound or metadata was temporarily unavailable. Tooltips are hosted at the themed application shell so the bounded editor frame cannot clip them or separate them from theme tokens. Its parse diagnostics remain advisory. Native formatting changes only the selected range or document, preserves comments through the checked formatter facade, and never executes or saves SQL.
