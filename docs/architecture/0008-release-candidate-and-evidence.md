...
Human/native results use versioned JSON records under `evidence/phase-5/`. The release-evidence auditor requires one exact clean source commit, all eight OS rows, all five artifacts, native performance samples, named manual reviews, the fixed five-working-day checklist, the opt-in beta threshold, exactly 101 verified requirements, exactly 20 verified acceptance criteria, and zero exceptions. Each package row records the complete core-journey categories, and one exact row per OS family records the network/vault/TLS journey. Accessibility results are retained per OS row for every required theme, width, and scale combination. Native performance summaries name their aggregation and are recomputed from at least 30 retained raw samples. The auditor parses the sole `SHA256SUMS` file and matches all five records rather than trusting a pass flag. Supporting evidence links must resolve to retained nonempty files inside the Phase 5 bundle and cannot point to templates or cite a final result record as its own proof.
...
- Aggregate pass assertions, declared sample counts, and checksum-verification flags cannot substitute for the structured supporting records the auditor recomputes or cross-matches.
