...
The query and result panes share a horizontal separator backed by `workspace.panel_sizes.results_percent`. The percentage defaults to 35, is clamped to 20–70, persists through the existing workspace snapshot, supports pointer drag plus Arrow/Home/End keyboard adjustment, and resets to 35 on double-click. At the upper result bound, the outer grid and CodeMirror share a three-and-a-half-row SQL viewport floor that includes possible horizontal-scrollbar space, so editor content remains scrollable instead of being clipped by the result pane. Only the selected result set renders its virtualized grid; additional sets use compact tabs. The results pane itself does not add a second vertical scrollbar, and export variants live behind one grouped control.
