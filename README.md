...
Current `master` extends the SQL editor's context-aware completion with loaded columns from statement tables and aliases, the last explicitly selected table as a pre-`FROM` fallback, and engine-aware SQLite/MySQL/MariaDB built-in functions such as `from_unixtime`, including MySQL-version gates. Arrow and Page keys navigate the open list, Tab accepts its selected item, and Enter continues to insert a SQL newline.

The current live `0.1.9` distribution envelope is Windows 11 x86-64, Linux x86-64, and macOS 13 or later on Intel and Apple silicon. Version 0.1.1 added the dedicated QueryNot signed-updater channel; existing updater-enabled installations can discover `0.1.9`, while the first installation remains manual. Native hardware checks, dogfood, and external beta remain explicit follow-up evidence until performed.
...
npm run release:validate-update-candidate -- --directory <candidate-dir> --tag v0.1.10 --report artifacts/candidate-validation.json
npm run release:prepare-update-publication -- --directory <candidate-dir> --output artifacts/publication --tag v0.1.10 --confirm publish-v0.1.10 --report artifacts/publication-plan.json
npm run release:verify-asset-metadata -- --release <release.json> --plan <publication-plan.json> --version 0.1.10 --tag v0.1.10 --source <commit> --draft true --report <report.json>
```
