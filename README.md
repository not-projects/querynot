...
Current `master` also contains the first compiled PostgreSQL adapter and connection/editor wiring. It uses PostgreSQL schema contexts, direct fail-closed TLS, bounded typed and array results, cancellation, transactions, table browsing, and guarded mutations through the same native contract. PostgreSQL 18.6 is the exact planned disposable conformance baseline; that fixture and release evidence are still pending, so PostgreSQL is not part of the live `0.1.13` support matrix. See [ADR 0019](docs/architecture/0019-postgresql-adapter-parity.md).
