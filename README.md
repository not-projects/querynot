...
The prepared `0.1.8` interface gives Connections, Schema, and local History a compact shared control tier, explicit connection states, quieter on-demand schema actions, readable query records, and progressively disclosed local-storage guidance. Database access, metadata loading, persisted history, and query reopening retain their existing behavior. The same source also streamlines signed candidate reuse and GitHub digest verification without weakening signature or exact-byte checks.

The current live `0.1.7` distribution envelope is Windows 11 x86-64, Linux x86-64, and macOS 13 or later on Intel and Apple silicon. Version 0.1.1 added the dedicated QueryNot signed-updater channel; existing updater-enabled installations can discover `0.1.7`, while the first installation remains manual. Native hardware checks, dogfood, and external beta remain explicit follow-up evidence until performed.
