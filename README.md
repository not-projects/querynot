...
The current live `0.1.15` distribution envelope is Windows 11 x86-64, Linux x86-64, and macOS 13 or later on Intel and Apple silicon. Version 0.1.1 added the dedicated QueryNot signed-updater channel; existing updater-enabled installations can discover `0.1.15`, while the first installation remains manual. Native hardware checks, dogfood, and external beta remain explicit follow-up evidence until performed.
...
The initial-release engine scope was SQLite, MySQL 5.7+ (with 5.7.44, 8.0, and 8.4 LTS as the tested lines), and MariaDB 10.11/11.4 LTS. MySQL 5.7 is a legacy compatibility target, not a recommendation to keep using an end-of-life server. PostgreSQL is the first post-initial-release adapter and ships as a development preview in `0.1.14`, with its dedicated conformance and support evidence still pending. The exact tested platform, database patch, authentication, and TLS matrix is published in the [compatibility matrix](docs/compatibility-matrix.md). See the [product requirements](docs/product-requirements.md) for the implementation and release gates.
...
npm run release:validate-update-candidate -- --directory <candidate-dir> --tag v0.1.16 --report artifacts/candidate-validation.json
npm run release:prepare-update-publication -- --directory <candidate-dir> --output artifacts/publication --tag v0.1.16 --confirm publish-v0.1.16 --report artifacts/publication-plan.json
npm run release:verify-asset-metadata -- --release <release.json> --plan <publication-plan.json> --version 0.1.16 --tag v0.1.16 --source <commit> --draft true --report <report.json>
```
...
`npm run test:conformance:phase3` verifies all five checksum-pinned MySQL/MariaDB archive fixtures through QueryNot's common adapter, including authentication, TLS/client identity, metadata, values, results, transactions, cancellation, and non-mutating estimated Explain. `npm run verify:phase3` runs this matrix plus the full local regression and desktop-build gate from a clean commit and retains commit-addressed reports.
