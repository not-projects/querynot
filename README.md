...
The current live `0.1.14` distribution envelope is Windows 11 x86-64, Linux x86-64, and macOS 13 or later on Intel and Apple silicon. Version 0.1.1 added the dedicated QueryNot signed-updater channel; existing updater-enabled installations can discover `0.1.14`, while the first installation remains manual. Native hardware checks, dogfood, and external beta remain explicit follow-up evidence until performed.
...
npm run release:validate-update-candidate -- --directory <candidate-dir> --tag v0.1.15 --report artifacts/candidate-validation.json
npm run release:prepare-update-publication -- --directory <candidate-dir> --output artifacts/publication --tag v0.1.15 --confirm publish-v0.1.15 --report artifacts/publication-plan.json
npm run release:verify-asset-metadata -- --release <release.json> --plan <publication-plan.json> --version 0.1.15 --tag v0.1.15 --source <commit> --draft true --report <report.json>
```
